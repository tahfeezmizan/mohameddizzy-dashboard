    ...
    return (
        <header className="sticky top-0 z-30 flex shrink-0 h-16 items-center justify-between gap-2 border-b bg-white px-2 sm:px-6">
            ...
            <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                    {locales.map((locale) => (
                        <Button key={locale.code} size="sm" onClick={() => changeLocale(locale.code)} className={`h-8 px-3 font-medium transition-all duration-200 ${currentLocale === locale.code ? "bg-[#155DFC] text-white hover:bg-[#0f46c0] border-[#155DFC]" : "bg-white text-slate-700 border-slate-200 hover:border-[#155DFC] hover:text-[#155DFC]"}`}>
