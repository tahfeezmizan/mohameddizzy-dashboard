    ...
    return (
        <Card className="col-span-1 lg:col-span-2 shadow-sm border-slate-200">
            <CardHeader className="pb-2">
                <CardTitle className="text-base font-bold text-slate-800">{t("ordersVsCompleted")}</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="h-62.5 w-full mt-4 flex items-center justify-center">
                    {isLoading ? (
                        <div className="flex flex-col items-center gap-2 text-slate-500">
                            ...
                            <p className="text-sm font-medium">{t("loadingChartData")}</p>
                        </div>
                    ) : data.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={data} margin={{ top: 5, right: 10, left: -20, bottom: 0 }} barGap={2}>
                                ...
                                <Bar dataKey="orders" fill="#3b82f6" radius={[2, 2, 0, 0]} barSize={20} name={t("totalOrders")} />
                                <Bar dataKey="completed" fill="#10b981" radius={[2, 2, 0, 0]} barSize={20} name={t("completedOrders")} />
                            </BarChart>
                        </ResponsiveContainer>
                    ) : (
