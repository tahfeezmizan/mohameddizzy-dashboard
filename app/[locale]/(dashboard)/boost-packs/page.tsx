        ...
        <div className="space-y-12">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <Button onClick={() => setOpen(true)} className="px-6 py-5.5 text-base! bg-blue-600 hover:bg-blue-700 w-full sm:w-auto text-white">
                    <Plus className="mr-2 h-4 w-4" /> {t("addNewPack")}
                </Button>
            </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-2 border-b pb-4">
                    ...
                    <h2 className="text-xl font-bold text-slate-800">{t("productBoost.title")}</h2>
                    <span className="bg-blue-100 text-blue-700 text-xs font-bold px-2.5 py-0.5 rounded-full uppercase">{t("productBoost.subtitle")}</span>
                </div>
                ...
                {productLoading ? (
                    ...
                    <div className="flex items-center justify-center min-h-50 text-red-500 font-medium text-sm">{t("productBoost.error")}</div>
                ) : productPacks.length > 0 ? (
                    <BoostPackCard data={productPacks} />
                ) : (
                    <div className="flex items-center justify-center min-h-37.5 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-slate-400 text-sm">{t("productBoost.noPacks")}</div>
                )}
            </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-2 border-b pb-4">
                    ...
                    <h2 className="text-xl font-bold text-slate-800">{t("shopBoost.title")}</h2>
                    <span className="bg-purple-100 text-purple-700 text-xs font-bold px-2.5 py-0.5 rounded-full uppercase">{t("shopBoost.subtitle")}</span>
                </div>
                ...
                {shopLoading ? (
                    ...
                    <div className="flex items-center justify-center min-h-50 text-red-500 font-medium text-sm">{t("shopBoost.error")}</div>
                ) : shopPacks.length > 0 ? (
                    <BoostPackCard data={shopPacks} />
                ) : (
                    <div className="flex items-center justify-center min-h-37.5 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-slate-400 text-sm">{t("shopBoost.noPacks")}</div>
                )}
            </div>
