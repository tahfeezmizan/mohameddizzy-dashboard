    ...
    return (
        <div className="">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 mb-2">Categories & Attributes</h1>
                    ...
                    <table className="w-full text-sm text-left">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {categories.map((category) => (
                                <tr key={category.name} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 min-w-50">
                                        <div className="flex flex-wrap gap-2">
                                            {category.subcategories.map((sub, idx) => (
                                                <span key={idx} className="bg-blue-50 text-blue-600 px-2.5 py-1 rounded-md text-xs font-semibold whitespace-nowrap">
                                                    {sub}
                                                </span>
                                            ))}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 font-bold text-slate-800">{category.listings}</td>
                                    <td className="px-6 py-4">
                                        <Badge variant="secondary" className="bg-emerald-50 text-emerald-600 hover:bg-emerald-50 font-medium whitespace-nowrap">
                                            {category.status}
                                        </Badge>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-3 font-medium">
                                            <Button onClick={() => setOpen(true)} className="bg-transparent text-blue-600 font-medium hover:text-blue-700">
                                                Edit
                                            </Button>
                                            <Button className="bg-transparent text-red-500 hover:text-red-700 transition-colors">Disable</Button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
